...
mapping_stats<-data.frame(flowcell = NA, 
                          n_processed = NA, 
                          n_assigned = NA, 
                          n_pseudoaligned = NA,
                          percent_barcoded = NA,
...
for (x in 1:length(in_dirs)) {
  ...
  mapping_add<-data.frame(flowcell = x, 
  ...
  # Add dataframe row of flowcell to overall dataframe of all flowcells
  mapping_stats<-rbind(mapping_stats, mapping_add)
  
  # View results once each flowcell has completed
  print(mapping_stats)
}

# Remove empty rows - used to create dataframes
mapping_stats<-mapping_stats[-1,]
sample_stats<-sample_stats[-1,]

# Save output tables
write.csv(mapping_stats, paste(base, "/", out_dirs, "/mapping_stats/flowcell_mapping_stats.csv", sep = ""), row.names = F)
write.csv(sample_stats, paste(base, "/", out_dirs, "/mapping_stats/sample_mapping_stats.csv", sep = ""), row.names = F)
...
# Save figure
sample_mapping<-ggplot(sample_stats, aes(x=sample, y=percent_bc_mapped, fill=factor(flowcell))) +
  geom_col(position = "dodge") +
  labs(title = "% of barcoded reads mapped", y = "%", x = "Sample", fill = "Flowcell") +
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)); sample_mapping
ggsave(paste(base, "/", out_dirs, "/mapping_stats/sample_mapping_stats.png", sep = ""), sample_mapping, bg = "white", width = 10, height=6)
...
df_labels <- df_long %>%
  ...
  left_join(mapping_stats, by = "flowcell") %>%
...
# Save figure
flowcell_mapping<-ggplot(df_long, aes(x=factor(flowcell), y=count, fill=status)) +
  geom_bar(stat="identity") +
  scale_fill_manual(values = c("darkgreen","orange","darkred"), labels = c("Mapped", "Barcoded, not mapped", "Not mapped or barcoded")) +
  geom_text(data = df_labels, aes(x = flowcell, y = total, label = label1), vjust = -2.6, inherit.aes = FALSE) +
  geom_text(data = df_labels, aes(x = flowcell, y = total, label = label2), vjust = -1.55, inherit.aes = FALSE) +
  geom_text(data = df_labels, aes(x = flowcell, y = total, label = label3), vjust = -0.5, inherit.aes = FALSE) +
  scale_y_continuous(expand = expansion(mult = c(0,0.2)))+
  theme_minimal() +
  labs(title = "Number of reads barcoded and mapped", y = "Number of reads", x = "Flowcell"); flowcell_mapping
ggsave(paste(base, "/", out_dirs, "/mapping_stats/flowcell_mapping_stats.png", sep = ""), flowcell_mapping, bg = "white", width = 10, height=6)
